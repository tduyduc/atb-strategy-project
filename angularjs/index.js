...
class DefaultDamage {
    static calculate(originalDamage, defense, variation = 0) {
        ...
        const variedDamage = Math.random() * variation * (Common.randomBool() ? 1 : -1);
        return Math.max(Math.floor(originalDamage * damageMultiplier + variedDamage), 0);
    }
}
/**
 ...
 * @see ICharacterClass
 */
class Character extends CharacterClass {
    constructor(arg) {
        super(arg);
...
const autoCharacterNames = characterClasses.reduce((result, { defaultCharacterNames }) => result.concat(defaultCharacterNames), []);
...
function mainControllerFunction($scope) {
    const scopeFunctions = {
        ...
        goToUnitDispatchWindow(force = false) {
            if (!force && !$scope.isCompletedClassLineup())
                return;
